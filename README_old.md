...
Google Sheets (주간 raw data, 외부에서 시간별 행 추가)
  ...
  → Gmail 알림 (심각+중간: 즉시 / 전체: 다음날 07:40 일일 리포트)
  → Google Sheets 결과 저장
...
| 이메일 알림 | 심각도별 분기, 반복 경고 태그, 사이클(60분) 단위 중복 억제 |
| AI Agent 일일 리포트 | AI Agent(Tool Use)가 자율적으로 데이터 조회 → 상황별 리포트 깊이 조절 (매일 07:40) |
| Power BI 대시보드 | 실시간 현황 / 이상 탐지 & AI 인사이트 / 장기 트렌드 (3페이지) |
| Power BI AI 시각화 | Key Influencers, Decomposition Tree, Smart Narrative (빌트인) |
| 데이터 무결성 | run_id(실행 추적), idempotency_key(중복 방지), notification_status(알림 상태) |
...
│       ├── data-gen/            # 테스트용 샘플 데이터 자동 생성, multi-day 지원 (예정)
...
├── pre-requirement/
│   ├── pre-requirement.txt      # 프로젝트 명세서 (v2.4 원본)
│   └── pre-requirement_v2.txt  # 프로젝트 명세서 (v3.2 최신)
...
| `/data-gen` | 1 | data_bank 자동 생성. 기본 1일(120행), `--days N`으로 최대 6일(720행). line_master 기반 정상 데이터 + 5가지 이상 시나리오. Power BI AI 시각화에 300행+ 필요 |
...
| D: 일일 리포트 (AI Agent) | Cron 매일 07:40 | AI Agent가 도구(Tool Use)로 자율 분석 → 상황별 리포트 깊이 조절 |

> **월요일 실행 순서**: D(07:40) → C(07:50) → 새 주 데이터(08:00). C가 production_week을 백업으로 이동하기 전에 D가 전날 데이터를 조회 완료해야 함.
...
자동화가 재실행되거나 실패해도 데이터가 중복되지 않고, 누락 여부를 추적할 수 있고, 왜 그런 결과가 나왔는지 설명 가능하게 하는 장치.

| 컬럼 | 대상 시트 | 형식 | 용도 |
|---|---|---|---|
| `run_id` | anomaly_log, error_log | `B_20260323_1110` | 어떤 실행에서 만들어진 데이터인지 추적. error_log와 조인 가능 |
| `idempotency_key` | anomaly_log | `L03_rule01_20260323_11` | 저장 전 같은 키 존재 시 스킵 → n8n 재실행 시 중복 방지 |
| `notification_status` | anomaly_log | `pending`/`sent`/`failed`/`skipped` | 알림 발송 상태 추적. Power BI에서 미발송 건 필터 |
...
| 2026-04-27 | v3.0~3.2: 설계서 전면 갱신 (pre-requirement_v2.txt). 데이터 무결성 장치 추가 (run_id, idempotency_key, notification_status). 워크플로 D 스케줄 08:00→07:40 (C→D 충돌 방지). 심각 재알림 30분→60분 (사이클 통일). /data-gen multi-day 옵션 추가 |
